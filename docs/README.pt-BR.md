...
> [!IMPORTANT]
> O package público está disponível como `@ninjadatabuilder/n8n-nodes-google-tag-manager@0.5.5`.
>
> [!WARNING]
...
Em **Settings → Community Nodes**, confirme a versão `0.5.5`.
...
- Não use `latest` em produção; fixe `@0.5.5`.
...
| `404` no package | Registry, versão ou rede incorretos | Use npm oficial e a versão `0.5.5`; não publique novamente a mesma versão |
...
- [Instalação detalhada](installation.pt-BR.md)
- [Guia em inglês](README.en.md)
- [Arquitetura, ambientes e sandbox](architecture.md)
