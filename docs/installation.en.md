...
> ✅ Current release: `@ninjadatabuilder/n8n-nodes-google-tag-manager@0.5.7`, updated for self-hosted n8n `2.39.7`. Previously stable validated packages: `0.5.5` for n8n `2.35.3` and `0.5.3` for n8n `2.32.5`.
>
...
The project reference environment was n8n `2.39.7` (also revalidate newer n8n releases in staging before production use).
...
In **Settings → Community Nodes**, confirm `0.5.7`. In the editor, search for `Google Tag Manager` and confirm:
...
| `npm 404` | Wrong registry, version, or network | Use `registry.npmjs.org` and `0.5.7` |
