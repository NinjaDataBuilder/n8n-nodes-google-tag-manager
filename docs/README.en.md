...
> [!IMPORTANT]
> The current release is `@ninjadatabuilder/n8n-nodes-google-tag-manager@0.5.7`, updated for self-hosted n8n `2.39.7`. Previously stable validated packages remain available for n8n `2.35.3` (`0.5.5`) and n8n `2.32.5` (`0.5.3`).
>
> [!WARNING]
...
Open **Settings → Community Nodes** and confirm version `0.5.7`.
...
- Do not use `latest` in production; pin `@0.5.7`.
...
| Package returns `404` | Wrong registry, version, or network | Use npm official and `0.5.5`; do not republish the same version |
